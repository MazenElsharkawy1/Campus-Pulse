...
const STAKEHOLDER_TYPE = 'council'
const STAKEHOLDER_ID = 3
...
function ReportsViewer({ userRole, onBack, filterType }: { userRole: string; onBack: () => void; filterType?: 'report' | 'query' | null }) {
  ...
  const handlePrint = async (elementId: string) => {
    ...
    await Promise.all(Array.from(images).map(img => {
      ...
      return new Promise((resolve, reject) => { img.onload = resolve; img.onerror = reject; if (img.onerror()) img.src = img.src })
    }))
